<?php
/**
 * lipold functions and definitions
 *
 ...
 */
function lipold_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on lipold, use a find and replace
		* to change 'lipold' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'lipold', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'lipold' ),
		)
	);
...
add_action( 'after_setup_theme', 'lipold_setup' );
 ...
 */
function lipold_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'lipold_content_width', 640 );
}
add_action( 'after_setup_theme', 'lipold_content_width', 0 );
 ...
 */
function lipold_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'lipold' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'lipold' ),
...
add_action( 'widgets_init', 'lipold_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function lipold_scripts() {
	wp_enqueue_style( 'lipold-style', get_template_directory_uri(). '/assets/css/main.css', array(), _S_VERSION );
	//wp_enqueue_style( 'lipold-style', get_stylesheet_uri(), array(), _S_VERSION );
	//wp_style_add_data( 'lipold-style', 'rtl', 'replace' );

	wp_enqueue_script( 'lipold-navigation', get_template_directory_uri() . '/assets/js/navigation.js', array(), _S_VERSION, true );
	wp_enqueue_script( 'lipold/header-mobile-sub-menu', get_template_directory_uri(). '/assets/js/header-mobile-sub-menu.js', array('jquery'), _S_VERSION );
	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'lipold_scripts' );
